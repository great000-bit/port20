const EXP = [
  {
    ...
    period:"2024 – Present",
    bullets:[
      "Manage Meta Ads and Google Ads campaigns, producing CEO-facing performance reports",
      ...
      "Handled workflow automation, Google Drive organisation, and formal meeting documentation",
    ],
  ...
  },
  {
    ...
    period:"2024",
    bullets:[
      "Built a full-stack Laravel 12 + React/TypeScript system for an Easter Bootcamp event registration",
      "Implemented Sanctum admin authentication, enrollment form, and admin CSV export dashboard",
      "Integrated Facebook Pixel CompleteRegistration event for conversion monitoring",
    ],
    tags:["Laravel","React","TypeScript","Sanctum","Facebook Pixel"],
  },
  {
    role:"WordPress Developer & UI/UX Designer",
    org:"Client Projects (NGO, International, Canadian)",
    loc:"Remote",
    period:"2022 – 2024",
    bullets:[
      "Built WordPress sites for YouthUp Global (international), Bellostylez (Canada), and Bourdillon Foundation (Nigeria)",
      "Delivered responsive, SEO-optimised builds using Elementor and Divi builders",
      "Designed UI/UX for Hair Brosh Saloon, AdsFirr, and client portfolios using Figma",
    ],
  ...
  },
  {
    ...
    bullets:[
      ...
      "Optimised for performance, Core Web Vitals, and SEO metadata on all deliverables",
    ],
    tags:["React","TypeScript","Vite","Framer","Portfolio Design"],
  },
];

export default function Experience() {
  return (
    <section id="experience" className="section bg-[#030303]">
      <div className="max-w-7xl mx-auto px-5">
        <div className="text-center mb-14">
          ...
          <p className="font-body text-white/45 mt-4">My professional journey and the work I've delivered</p>
        </div>

        {/* Timeline */}
        <div className="relative max-w-5xl mx-auto">
          {/* centre line — desktop */}
          <div className="absolute left-1/2 top-0 bottom-0 hidden md:block" style={{width:1,background:"rgba(111,4,20,0.25)",transform:"translateX(-50%)"}}/>

          {EXP.map((e,i)=>{
            const isLeft = i%2===0;
            return (
              <div key={i} className="relative mb-12 md:mb-16 flex flex-col md:block">
                {/* dot */}
                ...
                <div className={`md:w-[46%] ${isLeft?"md:mr-auto":"md:ml-auto"}`}>
                  <article className="glass rounded-2xl p-6 group transition-all duration-300"
                    ...
                    onMouseEnter={el=>el.currentTarget.style.borderColor="rgba(111,4,20,0.45)"}
                    ...
                    <p className="font-body text-xs text-white/35 mb-4">{e.loc}</p>
                    <ul className="space-y-2 mb-5">
                      {e.bullets.map((b,j)=>(
                        <li key={j} className="flex gap-2 text-sm text-white/55">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {e.tags.map(t=>(
                        <span key={t} className="text-xs px-2.5 py-1 rounded-full"
                          style={{background:"rgba(111,4,20,0.10)",border:"1px solid rgba(111,4,20,0.20)",color:"rgba(255,255,255,0.50)"}}>
                          {t}
                        </span>
                      ))}
                    </div>
