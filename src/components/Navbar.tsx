...
const Navbar = () => {
  ...
  // Navigation items with SEO-friendly labels
  const navItems = [
    { name: "About", href: "#about", ariaLabel: "About Great Emman-Wori - WordPress Developer" },
    { name: "Services", href: "#services", ariaLabel: "Web Development and Design Services" },
    { name: "Portfolio", href: "#portfolio", ariaLabel: "View Portfolio Projects" },
    { name: "Testimonials", href: "#testimonials", ariaLabel: "Client Testimonials and Reviews" },
    { name: "Contact", href: "#contact", ariaLabel: "Contact Great Emman-Wori" },
  ];
    ...
    >
      <div className="container mx-auto px-4 flex justify-between items-center">
        ...
        >
          ...
          <span className="relative z-10 text-portfolioTheme-primary text-4xl leading-none group-hover:text-portfolioTheme-accent transition-colors duration-300" aria-hidden="true">.</span>
          
          {/* Animated underline */}
          <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-gradient-to-r from-portfolioTheme-primary to-portfolioTheme-accent group-hover:w-full transition-all duration-500" aria-hidden="true"></span>
        </a>

        {/* Desktop Navigation with modern styling */}
        <ul className="hidden md:flex items-center space-x-2 bg-portfolioTheme-cardBg/50 backdrop-blur-sm rounded-full px-4 py-2 border border-gray-700/30" role="list">
          {navItems.map((item) => {
            const isActive = activeSection === item.href.substring(1);
            return (
              <li key={item.name} itemProp="name">
                <a 
                  href={item.href}
                ...
                >
                  ...
                  {!isActive && (
                    <span className="absolute inset-0 bg-portfolioTheme-primary/10 rounded-full scale-0 group-hover:scale-100 transition-transform duration-300" aria-hidden="true"></span>
                  )}
                  
                  {/* Active indicator dot */}
                  {isActive && (
                    <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-white rounded-full animate-pulse" aria-hidden="true"></span>
                  )}
                </a>
              </li>
            );
        ...
        <a
          ...
          aria-label="Contact Great Emman-Wori - Let's Talk"
        >
      ...
      <div 
        ...
        aria-hidden={!isOpen}
      >
        <ul className="container mx-auto px-4 py-6 flex flex-col space-y-2" role="list">
